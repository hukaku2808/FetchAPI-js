...
function getCoinData() {
    ...
    return fetch(coinUrl)
        ...
        .then(data => {
            return data; // Return api data successfully
        })
        .catch(error => {
        ...
        $('#coinTableBody').append(
            $('<tr class="content-row"></tr>').append(
                ...
                $('<td id="specific" class="text-left"></td>').append(
                    $('<div></div>').append(`<img src="${data[apiKey].image}" width="25"> <a href="/coin.html?${data[apiKey].id}">
            ${data[apiKey].name}</a>`)),
                $('<td class="text-right boldText"></td>').text("$" + number.format(data[apiKey].current_price.toFixed(2))),
                $('<td class="text-right"></td>').text("$" + number.format(data[apiKey].market_cap)),
                $('<td class="text-right"></td>').text("$" + number.format(data[apiKey].total_volume)),
                $('<td class="text-right"></td>').text(number.format(data[apiKey].circulating_supply.toFixed()) +
                    '  ' + data[apiKey].symbol.toUpperCase()),
                $(`<td class='${data[apiKey].price_change_percentage_24h >= 0 ? "text-success" : "text-danger"} 
        text-right'></td>`).text(Number(data[apiKey].price_change_percentage_24h).toFixed(2) + "%")
            )
        );
    };
};
...
//     let number = Intl.NumberFormat("en-US"); // Function Scope
